/* ============================================================
   APP.JS - COMPONENTE FUENTE DE ALIMENTACIÓN
...
const CONFIG_LOCAL = typeof CONFIG_AR !== 'undefined'
    ? CONFIG_AR
    : {
        componenteId: 'fuente-alimentacion',
        escalaInicial: 1,
        escalaMinima: 0.005,
        pasoEscala: 0.05,
...
const COMPONENTE_ID = CONFIG_LOCAL.componenteId || 'fuente-alimentacion';
const RUTA_RANKING_FIREBASE_DIRECTO = 'rankingFeriaFPBajoAragon';

const PREGUNTAS = [
    {
        texto: '¿Para qué sirve principalmente la fuente de alimentación?',
        opciones: [
            {
                texto: 'Para proporcionar energía eléctrica a los componentes del ordenador',
                correcta: true,
                feedbackCorrecto: 'Correcto. La fuente reparte energía a la placa base, CPU, discos, ventiladores y otros componentes.'
            },
            {
                texto: 'Para mostrar imágenes en la pantalla',
                correcta: false,
                feedbackIncorrecto: 'No es correcto. Las imágenes se muestran en el monitor y las procesa la tarjeta gráfica o la gráfica integrada.'
            },
            {
                texto: 'Para guardar documentos y programas',
                correcta: false,
                feedbackIncorrecto: 'No es correcto. Los documentos y programas se guardan en el disco duro o SSD.'
            }
        ]
    },
    {
        texto: '¿Qué hace la fuente con la corriente eléctrica que llega desde la pared?',
        opciones: [
            {
                texto: 'La transforma en voltajes adecuados para el ordenador',
                correcta: true,
                feedbackCorrecto: 'Muy bien. La fuente convierte la corriente de la red eléctrica en voltajes que los componentes del PC pueden usar.'
            },
            {
                texto: 'La convierte en memoria RAM',
                correcta: false,
                feedbackIncorrecto: 'No es correcto. La memoria RAM es otro componente diferente.'
            },
            {
                texto: 'La manda directamente al teclado para escribir más rápido',
                correcta: false,
                feedbackIncorrecto: 'No es correcto. El teclado no recibe la corriente principal de la fuente de esa manera.'
            }
        ]
    },
    {
        ...
        opciones: [
            {
                ...
                correcta: true,
                feedbackCorrecto: 'Correcto. El conector ATX de 24 pines es la alimentación principal de la placa base.'
            },
            {
                texto: 'El cable HDMI',
                correcta: false,
            ...
            {
                texto: 'El conector de audio frontal',
                correcta: false,
                feedbackIncorrecto: 'No es correcto. El audio frontal conecta auriculares y micrófono de la caja.'
            }
        ]
    },
    {
        texto: '¿Qué hay que comprobar antes de elegir una fuente de alimentación?',
        opciones: [
            {
                texto: 'Que tenga potencia suficiente y los conectores necesarios',
                correcta: true,
                feedbackCorrecto: 'Muy bien. La fuente debe tener potencia adecuada y conectores para placa base, CPU, discos y, si hace falta, tarjeta gráfica.'
            },
            {
                texto: 'Que sea del mismo color que el monitor',
                correcta: false,
                feedbackIncorrecto: 'No es correcto. El color no es lo importante. Hay que fijarse en potencia, calidad, conectores y compatibilidad.'
            },
            {
                texto: 'Que no tenga ventilador nunca',
                correcta: false,
                feedbackIncorrecto: 'No es correcto. Muchas fuentes tienen ventilador para refrigerarse correctamente.'
            }
        ]
    },
    {
        texto: '¿Por qué no se debe abrir una fuente de alimentación?',
        opciones: [
            {
                texto: 'Porque puede ser peligroso aunque el ordenador esté apagado',
                correcta: true,
                feedbackCorrecto: 'Correcto. La fuente puede contener componentes internos peligrosos. No debe abrirse sin formación técnica adecuada.'
            },
            {
                texto: 'Porque dentro está el sistema operativo',
                correcta: false,
                feedbackIncorrecto: 'No es correcto. El sistema operativo se guarda en el disco duro o SSD.'
            },
            {
                texto: 'Porque se borran las fotos del ordenador',
                correcta: false,
                feedbackIncorrecto: 'No es correcto. Abrir la fuente no borra fotos, pero sí puede ser peligroso para la seguridad.'
            }
        ]
    }
];
...
function comprobarFirebaseComponente() {
    if (typeof firebase === 'undefined') {
        console.warn('Firebase SDK no está cargado en la página de fuente.');
        return;
    }
    ...
    console.log('Firebase disponible en página fuente.');

    window.rankingDB.ref('.info/connected').on('value', function (snapshot) {
        console.log('Firebase conectado desde fuente:', snapshot.val());
    });
}
...
function configurarEventosMarcador() {
    ...
    marcador.addEventListener('markerFound', function () {
        aviso.innerHTML = '✅ Marcador detectado. Explora la fuente de alimentación.';
        aviso.style.display = 'block';
    });

    marcador.addEventListener('markerLost', function () {
        aviso.innerHTML = 'Enfoca de nuevo el marcador de la fuente de alimentación.';
        aviso.style.display = 'block';
    });
}
...
function cargarPregunta() {
    ...
    if (preguntaActual >= PREGUNTAS.length) {
        contenedor.innerHTML = `
            <p><strong>Has terminado las preguntas de la fuente de alimentación.</strong></p>
            <p>Ahora puedes continuar con la placa base.</p>
